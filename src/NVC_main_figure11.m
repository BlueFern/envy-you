%% Main script to plot FIG11 for NO paper
% radus and w_i for Glu / K / both / none
...
%% BK channel open probability
figure(1)
set(gcf, 'Position', [400 300 700 300]);
plot(time, state(:,ind.w_i),'LineWidth',1);
xlabel('Time (s)');
ylabel('w_i (dim.less)')
legend('1 - no input', '2 - only K+','3 - only Glu','4 - both')
hold all
